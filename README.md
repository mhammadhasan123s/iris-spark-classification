...
3. **EDA:** schema, missing value check, class balance, and feature distributions by species.
...
7. **Tuning:** `CrossValidator` with `ParamGridBuilder` (5-fold) for each model.
...
* *Setosa* is classified perfectly by every model, because it is linearly separable, as the EDA shows.
* All errors are *versicolor* vs *virginica* confusions, driven by overlapping petal measurements. This is visible in both the EDA distributions and the confusion matrices.
